    ...
    : ActionSetGetterBase<ReplaceAction>(gameData, actionGetter)
{
    ...
    private static readonly Dictionary<uint, uint[]> _replaceActions = new()
    {
        ...
        { 35920, [34634, 34635, 34640, 34641, 34642, 34643] }, //VPR Serpent's Tail PvE
        { 35921, [34636, 34638, 34644] }, //VPR Twinfang PvE
        { 35922, [34637, 34639, 34645] }, //VPR Twinblood PvE

        { 25764, [25765, 3545, 36948, 25882, 25768, 3543, 25769] }, //MNK Masterful Blitz PvE
    };
    ...
    public override Action[] GetActions(ReplaceAction item)
    {
        ...
        if (item.ReplaceAction1.Value is Action act1 && act1.RowId != 0)
        {
            actionList.Add(act1);
        }
        if (item.ReplaceAction2.Value is Action act2 && act2.RowId != 0)
        {
            actionList.Add(act2);
        }
        if (item.ReplaceAction3.Value is Action act3 && act3.RowId != 0)
        {
            actionList.Add(act3);
        }
